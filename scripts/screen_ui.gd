...
@onready var mobile_screen_ui =  $"../Mobile_UI"
...
func update_screen_ui():
	...
	if(Global.IS_MOBILE_SCREEN):
		update_powers_list_for_mobile_screen()
	else:
		update_powers_list()
...
func update_powers_list_for_mobile_screen():
	var player_powers_list = mobile_screen_ui.get_node("Control/Buttons_Container")
	# Remove icons that are no longer valid
	for child in player_powers_list.get_children():
		if child.name not in Global.PLAYER_POWERS_COLLECTION:
			player_powers_list.remove_child(child)
